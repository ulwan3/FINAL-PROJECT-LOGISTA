...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-lg mb-xl">
    ...
    <div class="glass-panel p-lg rounded-2xl border border-tertiary/20 relative overflow-hidden">
        ...
        <p class="text-3xl font-bold" style="color: var(--md-sys-color-tertiary, #cfbcff)">
            {{ $totalMutasi ?? 0 }}
        </p>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b border-outline-variant/30 bg-surface-container/40">
                    ...
                    <th class="py-3 px-4 font-label-bold text-on-surface-variant uppercase tracking-wider text-[11px] text-center">Stok Sebelum</th>
                    <th class="py-3 px-4 font-label-bold text-on-surface-variant uppercase tracking-wider text-[11px] text-center">Stok Sesudah</th>
            ...
            <tbody>
                @forelse($transaksiMasuk as $trx)
                    <tr class="border-b border-outline-variant/10 hover:bg-surface-container/50 transition-colors">
                        <td class="py-3 px-4 text-sm text-on-surface-variant whitespace-nowrap">
                            {{ $trx->created_at->format('d M Y, H:i') }}
                        </td>

                        <td class="py-3 px-4 text-sm font-bold text-on-surface">
                            {{ $trx->barang->nama_barang ?? '-' }}
                            <span class="text-xs text-on-surface-variant font-normal block">
                                {{ $trx->barang->kode_barang ?? '' }}
                            </span>
                        </td>

                        <td class="py-3 px-4 text-sm font-mono font-bold text-green-400 text-right">
                            +{{ $trx->jumlah }}
                        </td>

                        <td class="py-3 px-4 text-sm text-on-surface-variant text-center">
                            {{ $trx->total_stok_sebelum ?? '-' }}
                        </td>

                        <td class="py-3 px-4 text-sm font-bold text-on-surface text-center">
                            {{ $trx->total_stok_sesudah ?? '-' }}
                        </td>

                        <td class="py-3 px-4 text-sm text-on-surface-variant">
                            {{ $trx->user->name ?? 'Operator' }}
                        </td>

                        <td class="py-3 px-4 text-sm text-on-surface-variant max-w-[180px] truncate" title="{{ $trx->keterangan }}">
                            {{ $trx->keterangan ?: '-' }}
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="py-xl text-center text-on-surface-variant text-sm">
            ...
            <tbody>
                @forelse($transaksiKeluar as $trx)
                    <tr class="border-b border-outline-variant/10 hover:bg-surface-container/50 transition-colors">
                        ...
                        <td class="py-3 px-4 text-sm font-mono font-bold text-error text-right">
                            -{{ $trx->jumlah }}
                        </td>
            ...
            <tbody>
                @forelse($transaksiMutasi as $trx)
                    <tr class="border-b border-outline-variant/10 hover:bg-surface-container/50 transition-colors">
                        ...
                        <td class="py-3 px-4 text-sm font-mono font-bold text-right" style="color: var(--md-sys-color-tertiary, #cfbcff)">
                            {{ $trx->jumlah }}
                        </td>
                        ...
                        <td class="py-3 px-4 text-sm text-on-surface-variant max-w-[200px] truncate" title="{{ $trx->keterangan }}">
                            {{ $trx->keterangan ?: '-' }}
                        </td>
                    </tr>
                @empty
...
<script>
    ...
    if (['masuk', 'keluar', 'mutasi'].includes(hash)) {
        switchTab(hash);
    }
</script>
@endsection