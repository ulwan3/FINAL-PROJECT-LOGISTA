...
@section('content')
    <section class="flex flex-col gap-sm">
        ...
        <p class="font-body-sm text-on-surface-variant">
            Log aktivitas operasional dan keamanan dari seluruh pengguna sistem.
        </p>
    </section>

    <!-- Filter Section -->
    <form
        action="{{ route('audit.trail') }}"
        method="GET"
        class="glass-panel p-md rounded-xl flex flex-col md:flex-row gap-md items-end"
    >
        <div class="flex-1 w-full flex flex-col gap-xs">
            <label class="font-label-bold text-on-surface-variant text-[11px] uppercase tracking-wider ml-1">
                Rentang Waktu
            </label>
            ...
            <input
                ...
                class="w-full bg-surface-container-low border border-outline-variant/30 text-on-surface font-body-sm rounded-lg px-3 py-2 outline-none focus:border-primary"
            >
        </div>
        ...
        <div class="flex-1 w-full flex flex-col gap-xs">
            ...
            <select
                ...
                class="w-full bg-surface-container-low border border-outline-variant/30 text-on-surface font-body-sm rounded-lg px-3 py-2 outline-none focus:border-primary"
            >
                <option value="Semua Aktivitas">
                    Semua Aktivitas
                </option>

                <option value="masuk" {{ request('modul') == 'barang_masuk' ? 'selected' : '' }}>
                    Stok Masuk
                </option>

                <option value="keluar" {{ request('modul') == 'barang_keluar' ? 'selected' : '' }}>
                    Stok Keluar
                </option>

                <option value="mutasi" {{ request('modul') == 'mutasi' ? 'selected' : '' }}>
                    Mutasi
                </option>
            </select>
        </div>
        ...
        <button
            type="submit"
            class="w-full md:w-auto px-lg py-2 bg-primary text-on-primary font-label-bold rounded-lg hover:brightness-110 active:scale-95 transition-all"
        >
            Terapkan
        </button>
    </form>
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                <thead>
                    <tr class="bg-surface-container-high border-b border-outline-variant/30">
                        <th class="px-lg py-md font-label-bold text-on-surface-variant">
                            WAKTU (WIB)
                        </th>
                    ...
                    @forelse($transaksis as $trx)
                        <tr class="hover:bg-surface-container/50 transition-colors">
                            <td class="px-lg py-md">
                                ...
                                <div class="font-label-muted text-on-surface-variant mt-0.5">
                            ...
                            <td class="px-lg py-md">
                                ...
                                @elseif($trx->jenis == 'mutasi')
                                    <span class="px-2 py-1 rounded bg-blue-500/10 text-blue-400 font-label-bold text-[11px] border border-blue-500/20 flex items-center gap-1 w-max">
                                        <span class="material-symbols-outlined text-[14px]">
                                            sync_alt
                                        </span>
                                        Mutasi Stok
