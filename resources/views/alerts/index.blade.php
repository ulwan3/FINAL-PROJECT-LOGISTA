...
<div class="flex justify-between items-end">
    <div>
        ...
        <p class="text-on-surface-variant font-body-md mt-1">
            Sistem mendeteksi {{ $kritis->count() + $menipis->count() }} item yang membutuhkan pengisian ulang segera.
        </p>
    </div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-md mt-lg">
    <div class="glass-card p-lg rounded-xl flex flex-col justify-between h-32 border-l-4 border-error">
        ...
        <div class="flex items-baseline gap-sm">
            <span class="font-display text-[40px] text-error font-bold leading-none">
                {{ str_pad($kritis->count(), 2, '0', STR_PAD_LEFT) }}
            </span>
    ...
    <div class="glass-card p-lg rounded-xl flex flex-col justify-between h-32 border-l-4 border-tertiary">
        ...
        <div class="flex items-baseline gap-sm">
            <span class="font-display text-[40px] text-tertiary font-bold leading-none">
                {{ str_pad($menipis->count(), 2, '0', STR_PAD_LEFT) }}
            </span>
...
<div class="space-y-md mt-lg">
    <div class="grid grid-cols-12 gap-md px-lg text-on-surface-variant font-label-bold uppercase tracking-wider">
        ...
        <div class="col-span-2 text-center">Stok Minimum</div>
    ...
    @foreach($kritis as $item)
        @php
            $stokSaatIni = $item->total_stok_hitung ?? (($item->stok_gudang ?? 0) + ($item->stok_rak ?? 0));
        @endphp
        ...
        <div class="glass-card rounded-xl p-md group hover:bg-error-container/5 transition-all duration-300 border border-error/20">
            <div class="grid grid-cols-12 gap-md items-center">
                <div class="col-span-5 flex items-center gap-md">
                    ...
                    <div>
                        <h4 class="font-headline-sm text-body-md font-bold text-on-surface">
                            {{ $item->nama_barang }}
                        </h4>

                        <div class="flex gap-sm mt-xs">
                            <span class="font-label-bold px-sm py-[2px] bg-error-container/20 text-error rounded-full text-[10px]">
                                KOSONG
                            </span>

                            <span class="font-label-muted text-on-surface-variant text-[12px]">
                ...
                <div class="col-span-2 text-center">
                    <div class="flex flex-col">
                        <span class="text-headline-sm font-bold text-error">
                            {{ $stokSaatIni }} {{ $item->satuan }}
                        </span>
                    </div>
                </div>

                <div class="col-span-2 text-center">
                    <span class="text-body-md font-medium text-on-surface-variant">
            ...
            <div class="grid grid-cols-12 gap-md items-center">
                <div class="col-span-5 flex items-center gap-md">
                    ...
                    <div>
                        ...
                        <div class="flex gap-sm mt-xs">
                            <span class="font-label-bold px-sm py-[2px] bg-tertiary-container/20 text-tertiary rounded-full text-[10px]">
                                MENIPIS
                            </span>
                ...
                <div class="col-span-2 text-center">
                    <div class="flex flex-col">
                        <span class="text-headline-sm font-bold text-tertiary">
                            {{ $stokSaatIni }} {{ $item->satuan }}
                        </span>
                    </div>
                </div>
